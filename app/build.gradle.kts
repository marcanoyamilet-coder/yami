...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.unimarmovil.kxmpzq"
    minSdk = 24
    targetSdk = 34
    versionCode = 1
    versionName = "1.0"
...
afterEvaluate {
    ...
    tasks.findByName("assembleDebug")?.doLast {
        val srcFile = File(projectDirFile, "build/outputs/apk/debug/app-debug.apk")
        val destFile = File(rootDirFile, "UNIMAR_PreInscripcion.apk")
        if (srcFile.exists()) {
